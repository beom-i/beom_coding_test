...
### 배운 것
1) 차분하게 문제를 풀고 어떤 값을 기준점으로 잡고 어떤 값을 고정값으로 잡고 어떤 값을 가변값을 잡아야하는지 정의하고 풀면 조금 쉬웠다.
2) 이 문제에서는 조금 더 빠르게 푸려면 두가지 핵심 점이 있다.
   1) 다솜이보다 작은 값은 그냥 벡터에 넣지도 않기
   2) 다솜이보다 크지만 해당 벡터 내에서 최대값이 아니면 먼저 빼지않기 (=즉, 다솜이보다 큰 값 = 벡터에서 최대값 먼저 빼기)
