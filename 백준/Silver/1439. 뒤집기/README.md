...
### 배운것
1) 해당 문제는 1->0 0->1 둘다 가능하다에서 조금 고민했다. 왜 이렇게 했을까? 그러면 0이냐 1이냐는 중요하지 않고 어떻게 뒤집는 것을 빠르게 할 것인지에 대해 생각했다.
2) 해당 생각의 결과로는 짝수 홀수에 집중되었다. 어차피 뒤집을거 상관없으니까 홀수짝수 (홀수짝수의 기준은 순차탐색으로 문자열의 1->0 0->1로 바뀌는 순간들 카운트 숫자)
