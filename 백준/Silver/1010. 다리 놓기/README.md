...
1) 수학적으로 접근하지 못한 것 / 단순히 어떤 규칙이 있을거야 라는 생각을 갖고 접했다. 사실 수학문제이다 (조합)
2) 단순히 계산으로 풀 수 있다고 판단한 것.. 팩토리얼로 판단했다. 하지만 변수의 범위가 30까지 가능하면 최소 long long으로 해야하고 엄청나게 범위가 많아짐
3) 그렇다 해서 재귀로 n-1Cr-1 + n-1Cr 로 해보았는데 시간초과 나옴..

   정리하면, 계산 즉, 데이터가 한번에 몰리지 않고 곱하기와 나누기를 동시에 할 수 있는 방법으로 하나씩 줄여야 된다는 생각을 해야함.
