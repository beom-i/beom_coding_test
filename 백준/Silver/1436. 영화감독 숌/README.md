...
1) 브루트포스 알고리즘으로 해결하였다. 문제가 효율적으로 어떻게 풀지? 어떻게 규칙이 되어있지? 가 아닌 그냥 어떻게든 찾으면 되니까 while문을 써서 1~?까지 계속 탐색하면서 '666' 이 있는지 없는지 판단하고 있으면 count 세고 지극히 간단하게 생각하였다. -> 인터넷 브루트포스 를 보고 인지
