...
1) 첫 시도는 문자열에서 Ascii 코드가 작은 문자를 만나면 그 문자까지 자르고 reverse하고 총 두번하면 3개가 짤리지 않을까 생각했다.
2) 하지만, 이것은 reverse와 substr, sort 함수의 존재를 잘 몰랐던 것 모든 경우를 다 vector에 더하고 사전순으로 나열하면 Finish!
