...
### 배울 것
1) 구현 문제는 무조건 예외처리가 중요하다.최악의 상황과 최고의 상황 그니까 입력값이 제일 클때와 제일 작을 때를 생각해야한다.
   1) 출력이 다 대문자라 틀림
   2) 한자리수 일 때 생각 못함 -> 예외처리할때도 어디 부분에 할 지가 생각보다 많이 중요하다.
1) 잘한 것 : int로 받으면 안된다. string으로 바로 받은 것은 잘했다. 대신 이거 범위 잘 이해해야함.
