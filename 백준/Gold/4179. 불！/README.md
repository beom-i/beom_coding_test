...
### 배운 것
1) 진짜 너무 어렵다 ㅋㅋ .. 처음엔 불과 지훈이가 한 while문에 있어서 구현은 성공했지만, 시간초과가 떴다.
2) 다른 While문으로 불과 지훈을 따로 처리해서 지훈이 이동할 때 지훈 시간이 불시간과 같거나 불시간이 더 작다면 이미 불타는 것이기 때문에 종료시켜야한다.
3) 그리고 추가로 지훈이 계산할 때 외부로 범위가 나간다면 바로 종료시켜야한다.

   확실히 골드문제는 구현에 있어서 효율적인 구현 + 예외 처리로 빠르게 종료 등 고려할 게 많네..
