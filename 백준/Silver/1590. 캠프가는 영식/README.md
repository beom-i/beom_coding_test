...
1)
        ...
        해보았는데 안된다. 이유를 게시판에 올린 결과 답변해주셨다.
2)        
find(없는원소)를 하면 set.end()가 리턴되는데,
*set.end()를 하면 set의 원소 갯수가 나오는거 같아요. (컴파일러마다 다를 수 있을 것 같긴 합니다)
따라서 안전하게 set.count(T)==1 로 하시는게 어떨까 싶어요.
